...
class TestDataSet(unittest.TestCase):
    ...
    def test_collates_entry_names_datasets(self):
        for dataset_name, dataset in [
            ("GraphDataset", GraphDataset(self.hdf5_path, node_features=node_feats, edge_features=[Efeat.DISTANCE], target=targets.IRMSD)),
            ("GridDataset", GridDataset(self.hdf5_path, features=[Efeat.VDW], target=targets.IRMSD)),
        ]:
            entry_names = []
            for batch_data in DataLoader(dataset, batch_size=2, shuffle=True):
                entry_names += batch_data.entry_names

            assert set(entry_names) == set(
                ["residue-ppi-1ATN_1w:A-B", "residue-ppi-1ATN_2w:A-B", "residue-ppi-1ATN_3w:A-B", "residue-ppi-1ATN_4w:A-B"]
            ), f"entry names of {dataset_name} were not collated correctly"

    def test_datasets(self):
        dataset_graph = GraphDataset(hdf5_path=self.hdf5_path, subset=None, node_features=node_feats, edge_features=[Efeat.DISTANCE], target=targets.IRMSD)

        dataset_grid = GridDataset(hdf5_path=self.hdf5_path, subset=None, features=[Efeat.DISTANCE, Efeat.COVALENT, Efeat.SAMECHAIN], target=targets.IRMSD)
    ...
    def test_regression_griddataset(self):
        dataset = GridDataset(hdf5_path=self.hdf5_path, features=[Efeat.VDW, Efeat.ELEC], target=targets.IRMSD)
    ...
    def test_classification_griddataset(self):
        dataset = GridDataset(hdf5_path=self.hdf5_path, features=[Efeat.VDW, Efeat.ELEC], target=targets.BINARY)
    ...
    def test_inherit_info_training_griddataset(self):
        dataset_train = GridDataset(
            hdf5_path=self.hdf5_path, features=[Efeat.VDW, Efeat.ELEC], target=targets.BINARY, target_transform=False, task=targets.CLASSIF, classes=None
        )

        dataset_test = GridDataset(hdf5_path=self.hdf5_path, train=False, dataset_train=dataset_train)
    ...
    def test_size_graphdataset(self):
        ...
        dataset = GraphDataset(hdf5_path=hdf5_paths, node_features=node_feats, edge_features=[Efeat.DISTANCE], target=targets.BINARY)
        n = 0
    ...
    def test_hdf5_to_pandas_graphdataset(self):
        hdf5_path = "tests/data/hdf5/train.hdf5"
        dataset = GraphDataset(hdf5_path=hdf5_path, node_features="charge", edge_features=["distance", "same_chain"], target="binary")
        ...
        dataset = GraphDataset(hdf5_path=hdf5_path, node_features="charge", edge_features=["distance", "same_chain"], target="binary", subset=keys[2:])
        dataset.hdf5_to_pandas()
    ...
    def test_logic_train_graphdataset(self):
        ...
        dataset_train = GraphDataset(hdf5_path=hdf5_path, target="binary")

        dataset_test = GraphDataset(hdf5_path=hdf5_path, target="binary", train=False, dataset_train=dataset_train)
        ...
        with self.assertRaises(TypeError):
            GraphDataset(hdf5_path=hdf5_path, target="binary", train=False)
                    ...
                    for i in range(arr.shape[1]):
                        ...
                        if transform:
                            ...
                            assert not np.allclose(
                                np.nanmean(transf_features_dict.get(orig_feat + f"_{i}")), np.nanmean(features_dict.get(orig_feat + f"_{i}"))
                            )
    ...
    def test_features_transform_logic_graphdataset(self):
        ...
        dataset_test = GraphDataset(hdf5_path=hdf5_path, train=False, dataset_train=dataset_train, target="binary")
        ...
        dataset_test = GraphDataset(hdf5_path=hdf5_path, train=False, dataset_train=dataset_train, features_transform=other_feature_transform, target="binary")
    ...
    def test_invalid_value_features_transform(self):
        ...
        transf_dataset = GraphDataset(hdf5_path=hdf5_path, target="binary", features_transform=features_transform)
    ...
    def test_incompatible_dataset_train_type(self):
        dataset_train = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", edge_features=[Efeat.DISTANCE, Efeat.COVALENT], target=targets.BINARY)

        # Raise error when val dataset don't have the same data type as train dataset.
        with pytest.raises(TypeError):
            GridDataset(hdf5_path="tests/data/hdf5/1ATN_ppi.hdf5", train=False, dataset_train=dataset_train)
