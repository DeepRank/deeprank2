...
from deeprank_gnn.models.metrics import OutputExporter, TensorboardBinaryClassificationExporter


def _model_base_test(hdf5_path, model_class,
                     node_features, edge_features,
                     task, target, metrics_exporters):
...
class TestNeuralNet(unittest.TestCase):
    ...
    def test_ginet(self):
        _model_base_test("tests/hdf5/1ATN_ppi.hdf5", GINet,
                         ['type', 'polarity', 'bsa', 'depth', 'hse', 'ic', 'pssm'], ['dist'],
                         'reg', 'irmsd', [OutputExporter(self.work_directory)])

        assert len(os.listdir(self.work_directory)) > 0

    def test_ginet_class(self):
        _model_base_test("tests/hdf5/variants.hdf5", GINet,
                         ['size', 'polarity', 'sasa', 'ic', 'pssm'], ['dist'],
                         'class', 'bin_class', [TensorboardBinaryClassificationExporter(self.work_directory)])

        assert len(os.listdir(self.work_directory)) > 0

    def test_fout(self):
        _model_base_test("tests/hdf5/1ATN_ppi.hdf5", FoutNet,
                         ['type', 'polarity', 'bsa', 'depth', 'hse', 'ic', 'pssm'], ['dist'],
                         'reg', 'irmsd', [])

    def test_sgat(self):
        _model_base_test("tests/hdf5/1ATN_ppi.hdf5", sGAT,
                         ['type', 'polarity', 'bsa', 'depth', 'hse', 'ic', 'pssm'], ['dist'],
                         'reg', 'irmsd', [])
