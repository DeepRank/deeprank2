...
def base_test(model_path, trainer: Trainer, lossfunction=None, override=False):
    ...
    with warnings.catch_warnings(record=UserWarning):
        ...
        trainer_pretrained = Trainer(neuralnet=NaiveNetwork, dataset_test=trainer.dataset_train, pretrained_model=model_path)
...
class TestLosses(unittest.TestCase):
    ...
    def test_classif_weighted(self):
        dataset = GraphDataset(hdf5_path, target=targets.BINARY)
        trainer = Trainer(neuralnet=NaiveNetwork, dataset_train=dataset, class_weights=True)
    ...
    def test_regress_default(self):
        dataset = GraphDataset(hdf5_path, target="BA", task="regress")
        trainer = Trainer(
            neuralnet=NaiveNetwork,
            dataset_train=dataset,
        )
