...
def querycollection_tester(n_queries = 10, feature_modules = None, processes = 1, combine_output = True):
    """
    ...
    try:
        ...
        output_paths = collection.process(prefix, feature_modules, processes, combine_output)
...
def test_querycollection_process_combine_output_false():
    ...
    _, output_directory, output_paths = querycollection_tester(processes = processes, combine_output = combine_output)

    assert len(output_paths) == processes
