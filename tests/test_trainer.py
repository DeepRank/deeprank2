...
def _model_base_test(
    ...
    with warnings.catch_warnings(record=UserWarning):
        ...
        Trainer(model_class, dataset_train, dataset_val, dataset_test, pretrained_model=save_path)
...
class TestTrainer(unittest.TestCase):
    ...
    def test_grid_regression(self):
        dataset = GridDataset(hdf5_path="tests/data/hdf5/1ATN_ppi.hdf5", subset=None, features=[Efeat.VDW], target=targets.IRMSD, task=targets.REGRESS)
    ...
    def test_grid_classification(self):
        dataset = GridDataset(hdf5_path="tests/data/hdf5/1ATN_ppi.hdf5", subset=None, features=[Efeat.VDW], target=targets.BINARY, task=targets.CLASSIF)
    ...
    def test_incompatible_pretrained_no_test(self):
        with pytest.raises(ValueError):
            dataset = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", clustering_method="mcl", target=targets.BINARY)
            trainer = Trainer(
                neuralnet=GINet,
                dataset_train=dataset,
            )
            ...
            with warnings.catch_warnings(record=UserWarning):
                ...
                Trainer(neuralnet=GINet, dataset_train=dataset, pretrained_model=self.save_path)
    ...
    def test_no_valid_full_train(self):
        dataset = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", clustering_method="mcl", target=targets.BINARY)
        trainer = Trainer(neuralnet=GINet, dataset_train=dataset, val_size=0)
        trainer.train(batch_size=1, best_model=False, filename=None)
    ...
    def test_optim(self):
        ...
        with warnings.catch_warnings(record=UserWarning):
            ...
            trainer_pretrained = Trainer(neuralnet=NaiveNetwork, dataset_test=dataset, pretrained_model=self.save_path)
    ...
    def test_dataset_equivalence_no_pretrained(self):
        ...
        dataset_val = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", train=True, dataset_train=dataset_train)
        dataset_test = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", train=True, dataset_train=dataset_train)
        with pytest.raises(ValueError):
            Trainer(
                neuralnet=GINet,
                dataset_train=dataset_train,
        ...
        dataset_train_other = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", edge_features=[Efeat.SAMECHAIN, Efeat.COVALENT], target="BA", task="regress")
        dataset_val = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", train=False, dataset_train=dataset_train)
        dataset_test = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", train=False, dataset_train=dataset_train)
        with pytest.raises(ValueError):
            Trainer(neuralnet=GINet, dataset_train=dataset_train_other, dataset_val=dataset_val)
        with pytest.raises(ValueError):
            Trainer(neuralnet=GINet, dataset_train=dataset_train_other, dataset_test=dataset_test)
    ...
    def test_dataset_equivalence_pretrained(self):
        ...
        dataset_train = GraphDataset(
            hdf5_path="tests/data/hdf5/test.hdf5", edge_features=[Efeat.DISTANCE, Efeat.COVALENT], clustering_method="mcl", target=targets.BINARY
        )
    ...
    def test_invalid_trainsize(self):
        ...
        test_cases = [
            1.0,
            n,  # cannot be 100% validation data
            -0.5,
            -1,  # no negative values
            1.1,
            n + 1,  # cannot use more than all data as input
        ]
    ...
    def test_invalid_cuda_ngpus(self):
        dataset_train = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5")
        dataset_val = GraphDataset(hdf5_path="tests/data/hdf5/test.hdf5", train=False, dataset_train=dataset_train)

        with pytest.raises(ValueError):
            Trainer(neuralnet=GINet, dataset_train=dataset_train, dataset_val=dataset_val, ngpu=2)
    ...
    def test_invalid_no_cuda_available(self):
        if not torch.cuda.is_available():
            ...
            with pytest.raises(ValueError):
                Trainer(neuralnet=GINet, dataset_train=dataset_train, dataset_val=dataset_val, cuda=True)
