...
![DeepRank2 framework overview. 3D coordinates of protein structures are extracted from PDB files and converted into graphs, using either an atomic or a residue level, depending on the user’s requirements. Then, the data are enriched with geometrical and physicochemical information, and eventually mapped to 3D grids, before finally storing them into HDF5 files. The processed data can be used in the pre-implemented DL pipeline for training PyTorch networks and computing predictions.\label{fig:flowchart}](deeprank2.png){ width=100% }
