...
with h5py.File("<hdf5_path.hdf5>", "r") as hdf5:
    ...
    node_features = list(hdf5[ids[0]][groups.NODE]) 
    # List of all edge features
    edge_features = list(hdf5[ids[0]][groups.EDGE])
    # List of all edge targets
    targets = list(hdf5[ids[0]][targets.VALUES])

    # BSA feature for ids[0], numpy.ndarray
    node_feat_polarity = hdf5[ids[0]][groups.EDGE][Nfeat.BSA][:] 
