...
class Query():
    ...
    def __init__(self, model_id: str, targets: Optional[Dict[str, Union[float, int]]] = None):
        ...
        Args:
            ...
            targets(Dict[str, Union[float, int]]): target values associated with the query, defaults to None.
        """
...
class QueryCollection:
    ...
    def process( # pylint: disable=too-many-arguments
        ...
        """
        Args:
            prefix(str): prefix for the output files. Defaults to None, which sets ./processed-queries- prefix.

            feature_modules(List[ModuleType]): list of features' modules used to generate features.
                ...
                Defaults to None, which means that all available modules in deeprankcore.features are used to
                generate the features. 
            
            cpu_count(int): how many processes to be run simultaneously.
                Defaults to None, which takes all available cpu cores.

            combine_output(bool): for combining the hdf5 files generated by the processes. Defaults to True.
