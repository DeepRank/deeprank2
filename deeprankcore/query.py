...
class SingleResidueVariantResidueQuery(Query):
    ...
    def build(self, feature_modules: List[ModuleType], include_hydrogens: Optional[bool] = False) -> Graph:
...
class SingleResidueVariantAtomicQuery(Query):
    ...
    def _get_atom_node_key(atom) -> str:
        """Pickle has problems serializing the graph when the nodes are atoms,so use this function to generate an unique key for the atom"""
