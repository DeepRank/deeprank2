...
def build_atomic_graph( # pylint: disable=too-many-locals
    ...
    for atom1_index, atom2_index in numpy.transpose(numpy.nonzero(neighbours)):
        if atom1_index != atom2_index:
            ...
            node1 = Node(atom1)
            node2 = Node(atom2)
            node1.features[nodefeats.POSITION] = atom1.position
            node2.features[nodefeats.POSITION] = atom2.position
    ...
    for atom1_index, atom2_index in numpy.transpose(numpy.nonzero(neighbours)):
        if atom1_index != atom2_index:
            ...
            if residue1 != residue2:
                ...
                node1.features[nodefeats.POSITION] = numpy.mean(
                    [atom.position for atom in residue1.atoms], axis=0
                )
                node2.features[nodefeats.POSITION] = numpy.mean(
